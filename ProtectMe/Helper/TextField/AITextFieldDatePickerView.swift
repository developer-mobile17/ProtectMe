//
//  AITextFieldDatePickerView.swift
//  AI_TextField
//
...
open class AITextFieldDatePickerView: AIBaseTextField {
    ...
    public var pickerViewDateDoneHandler: ((_ txtField: AITextFieldDatePickerView, _ selectedDate: Date, _ selectedDateInString: String) -> Void)?
    
    /// For setting maximum year range allowed from currentDate in date picker
    public var maxYear: Int = 10 {
        didSet {
    ...
    func commonInitialize() {
        ...
        buttonNext.addTarget(self, action: #selector(self.buttonDonePressed1(_:)), for: .touchUpInside)
        ...
        buttonCancel.addTarget(self, action: #selector(self.buttonCancelPressed1(_:)), for: .touchUpInside)
    ...
    @objc private func buttonDonePressed1(_ snnder: UIButton) {
        ...
        if let handler = self.pickerViewDateDoneHandler {
