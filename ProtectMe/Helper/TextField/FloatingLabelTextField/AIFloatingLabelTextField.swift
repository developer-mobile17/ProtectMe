//
//  AIFloatingLabelTextField.swift
//  AI_TextField
//
//  Created by kavin_macbook-1 on 02/01/19.
//

import UIKit

open class AIFloatingLabelTextField: AIBaseTextField {
    ...
    private func updatePlaceholder() {
        ...
        self.titleLabel?.font = self.placeholderFont ?? UIFont.init(name: (self.font?.fontName)!, size: (self.font?.pointSize)! - 2)
    }
    ...
    private final func setupTextField() {
        borderStyle = .none
        createTitleLabel()
        createLineView()
        updateColors()
    ...
    public override func updateTextAligment() {
        super.updateTextAligment()
        if isLTRLanguage {
            textAlignment = .left
    ...
    private func addEditingChangedObserver() {
        self.addTarget(self, action: #selector(AIFloatingLabelTextField.editingChanged), for: .editingChanged)
    }
    ...
    private func createTitleLabel() {
        ...
        titleLabel.font = self.placeholderFont ?? .systemFont(ofSize: 13)
    ...
    public override func updateTextField() {
        ...
        self.lineColor = self.config.bottomLineNormalColor ?? self.lineColor
        self.selectedLineColor = self.config.bottomLineEditingColor ?? self.selectedLineColor
        self.placeholderColor = self.config.placeholderColor ?? self.placeholderColor
        self.titleColor = self.config.placeholderColor ?? self.titleColor
        self.selectedTitleColor = self.config.floatingLabelEditingColor ?? self.selectedTitleColor
        self.lineColor = self.config.bottomLineNormalColor ?? self.lineColor
        self.selectedLineColor = self.config.bottomLineEditingColor ?? self.selectedLineColor
    }
}
...
extension AIFloatingLabelTextField {
